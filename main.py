...
def get_start_kb():
    buttons = [
        [InlineKeyboardButton(text="🆓 Náš bezplatný kanál", url="https://t.me/CatLite")],
        [InlineKeyboardButton(text="💳 Přejít k platbě kurzu", callback_data="go_to_pay")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def get_pay_options():
    buttons = [
        [InlineKeyboardButton(text=f"Kurz ({BASE_PRICE} USDT)", callback_data=f"pay_{BASE_PRICE}")],
        [InlineKeyboardButton(text="Kurz + Dýško ☕ (7.0 USDT)", callback_data="pay_7.0")],
        [InlineKeyboardButton(text="✍️ Jiná částka (Kurz + tvé dýško)", callback_data="pay_custom")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_check_kb(url, invoice_id):
    buttons = [
        ...
        [InlineKeyboardButton(text="🔄 Zkontrolovat platbu", callback_data=f"check_{invoice_id}")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def start(message: Message, state: FSMContext):
    await state.clear()
    text = (
        "<b>👋 Ahoj! Rád tě vidím v CatPay!</b>\n\n"
...
async def go_to_pay_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Vyber si variantu platby. Kurz včetně poplatků služby stojí <b>{BASE_PRICE} USDT</b>.\n"
...
async def pay_cmd(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Vyber si variantu platby. Kurz včetně poplatků služby stojí <b>{BASE_PRICE} USDT</b>.\n"
        "Můžeš si vybrat hotovou variantu nebo zadat vlastní částku! 👇",
        reply_markup=get_pay_options(),
        parse_mode="HTML"
    )
...
async def pay_custom_cmd(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        f"Zadej částku v USDT, kterou chceš poslat (minimum <b>{BASE_PRICE}</b>):\n\n"
        "Příklad: 5.5 nebo 15",
        parse_mode="HTML"
    )
...
async def process_custom_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        if amount < BASE_PRICE:
            await message.answer(f"❌ Částka nesmí být menší než цена kurzu ({BASE_PRICE} USDT). Zkus to znovu:")
            return
        amount = round(amount, 2)
        invoice = await crypto.create_invoice(asset="USDT", amount=amount)
        await message.answer(
            f"Částka k platbě: <b>{amount} USDT</b>\n\n"
        ...
        await message.answer("❌ Zadej prosím platné číslo. Například: 6.5")

@dp.callback_query(F.data.startswith("pay_"))
async def create_invoice_handler(callback: CallbackQuery):
...
async def check_invoice_handler(callback: CallbackQuery):
    ...
    if invoice and invoice.status == "paid":
        ...
        try:
            link = await bot.create_chat_invite_link(chat_id=CHANNEL_ID, member_limit=1, name=f"User_{user.id}")
